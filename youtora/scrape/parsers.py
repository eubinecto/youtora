...
class ChannelRawParser:
    ...
    def _ext_video_id_list(cls, uploads_html: str) -> List[str]:
        ...
        video_id_list = list()
        # is this div tag?
        uploaded_thumbs = soup.find_all('div', cls.UPLOADED_THUMB_CLASS)
...
class CaptionsRawParser:
    ...
    @classmethod
    def parse(cls, caption_raw: CaptionsRaw) -> List[CaptionData]:
        logger = logging.getLogger("parse")
        auto_info: dict = json.loads(caption_raw.auto_captions_json)
        manual_info: dict = json.loads(caption_raw.manual_captions_json)
        video_id = caption_raw.video_id
        captions = list()
    ...
    def _ext_caption(cls, caption_info: dict,
        ...
        caption_url = None
        if lang_code in caption_info:  # manual exists
...
class TracksRawParser:
    ...
    def parse(cls, tracks_raw: TracksRaw) -> List[TrackData]:
        ...
        tracks_xml = html.unescape(tracks_raw.xml)
...
class VideoRawParser:
    ...
    def parse(cls, video_raw: VideoRaw) -> VideoData:
        info: dict = json.loads(video_raw.video_info_json)
        # access the results
        vid_id = info['id']
        title = info['title']
        ...
        manual_captions_info = json.loads(video_raw.captions_raw.get_col('manual_captions_info_json'))
        auto_captions_info = json.loads(video_raw.captions_raw.get_col('auto_captions_info_json'))
        ...
        video = VideoData(id=vid_id, title=title, url=vid_url,
                          ...
                          category=category, manual_captions_info=manual_captions_info,
